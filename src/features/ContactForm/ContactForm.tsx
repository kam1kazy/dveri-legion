...
import { type ContactFormStep, useContactForm } from './model/useContactForm';
...
export const ContactForm = () => {
  ...
  return (
    <form ref={formRef} className={style.form} onSubmit={handleSubmit} noValidate>
      <nav className={style.nav} aria-label="Шаги формы">
          ...
          {formSections.map((section, index) => {
            ...
            return (
              <li key={section.id}>
                <button
                  type="button"
                  className={`${style.navLink} ${isCurrent ? style['navLink--active'] : ''}`}
                  onClick={() => goToStep(index as ContactFormStep)}
                  disabled={!isReached}
      ...
      <div className={style.fields}>
        {step === 0 && (
          <section className={style.section} aria-labelledby="step-information">
            <h3 id="step-information" className={style.sectionTitle}>
              Информация
            </h3>
        ...
        {step === 1 && (
          <section className={style.section} aria-labelledby="step-contacts">
            <h3 id="step-contacts" className={style.sectionTitle}>
              Контакты
            </h3>
        ...
        {step === 2 && (
          <section className={style.section} aria-labelledby="step-message">
            <h3 id="step-message" className={style.sectionTitle}>
              Сообщение
            </h3>
        ...
        <div className={style.submit}>
          {step < 2 ? (
            <Button type="button" text="Следующий шаг" filled dark onClick={goNext}>
              <ArrowRightWhiteIcon style={{ fontSize: '1.25rem', marginLeft: 8 }} />
            </Button>
          ) : (
