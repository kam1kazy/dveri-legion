export { Icon, type IIcon } from './Icon';

export { default as ArrowCarouselIcon } from './svg/arrow-carousel.svg';
export { default as ArrowDownIcon } from './svg/arrow-down.svg';
export { default as ArrowRightWhiteIcon } from './svg/arrow-right-white.svg';
export { default as ArrowRightIcon } from './svg/arrow-right.svg';
export { default as CloseIcon } from './svg/close.svg';
export { default as MenuIcon } from './svg/menu.svg';
